...
def check_band_spread_claim() -> None:
    ...
    'roughly two to over three times' the between-system HM spread at 50 ms.

    Recomputed from the artifacts; fails the table build if the prose claim
    would be false (min ratio must sit in [1.5, 2.5), max ratio in [3, 4)).
    """
    hms, bands = [], []
    for sysname in S.SYSTEMS:
        sw = S.sweep(sysname)
    ...
    assert 1.5 <= lo < 2.5 and 3.0 <= hi < 4.0, (
        "letter claim 'roughly two to over three times the spread' no longer "
